...
namespace com.github.benpocalypse.circuitent
{
    public class Kicad_sch
    {
        ...
        public void Print()
        {
            ...
            foreach(WireSegment w in Wires)
            {
                ...
                while ((line = dis.read_line (null)) != null)
                {
                    ...
                    if(tokens.length > 0)
                    {
                        switch(tokens[0].up ())
                        {
                            ...
                            // FIXME - this needs to be a literal string (whatever the syntax) that is "$ENDDESCR"
                            case """$ENDDESCR""":
                                fMode = FileMode.UNKNOWN;
                                break;

                            // FIXME - this needs to be a literal string (whatever the syntax) that is "$COMP"
                            case """$COMP""":
                            ...
                            case "WIRE":
                                if(tokens[1].up() == "WIRE")
                                {
                                    ...
                                    Wires.append(new WireSegment(int.parse(wiretokens[0]),
                            ...
                            default:
                                stdout.printf("Unhandled token: %s\r\n", tokens[0].up ());
                                break;
                        }
                    }
        ...
        private bool ParseComponent(DataInputStream dis)
        {
            ...
            if(line != null)
            {
                ...
                while(tokens[0].up().contains ("ENDCOMP") != true)
                {
    ...
    public class WireSegment
    {
        ...
        public WireSegment(int sx, int sy, int ex, int ey)
        {
            StartX = sx;
            StartY = sy;
            EndX = ex;
